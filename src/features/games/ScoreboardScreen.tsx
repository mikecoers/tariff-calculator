...
export default function ScoreboardScreen() {
  ...
  if (loading || !game) return <div className="p-4 text-phil-creamDim">Loading…</div>;
  ...
  return (
    <div className="h-dvh w-full flex flex-col safe-top safe-bottom overflow-hidden">
      <div className="px-4 py-2 flex items-center justify-between text-xs text-phil-creamDim">
      ...
      <div className="flex-1 flex flex-col justify-center px-4">
        ...
        <div className="mt-6 text-center">
          <div className="text-phil-creamDim uppercase tracking-widest text-xs">Inning</div>
          <div className="font-display text-[120px] leading-none tracking-widest text-phil-cream">
            {game.halfInning === 'top' ? '▲' : '▼'}
            {game.inning}
          </div>
        </div>
        ...
        <div className="mt-6 flex items-center justify-center gap-2 text-xs text-phil-creamDim">
    ...
    >
      <div
        className={`uppercase tracking-widest text-sm ${
          active ? (tone === 'ok' ? 'text-ump-ok' : 'text-ump-warn') : 'text-phil-creamDim'
        } font-bold truncate`}
      >
        {label}
      </div>
      <div className="font-mono text-[96px] leading-none font-black text-phil-cream">{value}</div>
      {active && <div className="text-[10px] text-phil-creamDim mt-1">AT BAT</div>}
    </div>
  );
}
...
function Cell({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-2xl border border-ump-line p-3 text-center">
      <div className="uppercase text-[10px] tracking-widest text-phil-creamDim">{label}</div>
      <div className="font-mono text-5xl font-black text-phil-cream">{value}</div>
    </div>
  );
}
